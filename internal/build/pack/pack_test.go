...
// buildRoot points at the internal/build directory containing testdata/.
...
func helloNoarchStaged(t *testing.T) string {
	return filepath.Join(buildRoot(t), "testdata", "cases", "hello-noarch", "staged")
}
...
func loadTestSigningKey(t *testing.T) ed25519.PrivateKey {
	t.Helper()
	path := filepath.Join(buildRoot(t), "testdata", "keys", "test-signing.ed25519")
...
// and confirm the signature verifies. If this passes, pack is signing
// the right bytes.
