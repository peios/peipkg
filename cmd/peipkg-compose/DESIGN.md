...
# The producer half of the §5.14 two-key layout waiver. Without it a
# package declaring special_system_package is still held to the payload
# path rules; without the declaration this flag alone waives nothing.
bypass_path_restrictions = false

# A named root the image carries beside the anchor. Each becomes a
# named_root registry entry, so the booted system resolves `--root
...
- **`[[package]]`** entries are top-level *wants*, each a name and a
  ...
  `repository = "official"` pins where a package is sourced from, and an
  optional `root = "initramfs"` names a declared `[[root]]` to install
  into, exactly as `--root` does for `peipkg install`. Package identity
  in the manifest layer is the **(root, name) pair**, so the same
  package may appear in more than one root with different constraints.
- **`[[root]]`** entries declare the named roots the image carries. A
  root a `[[package]]` or a dependency `root` placement names must be
  declared here.
- **`bypass_path_restrictions`** is the operator half of §5.14's
  two-key waiver; see "The output root".
...
manifest_digest = "4f1c…"   # 64 lowercase hex, required

# One entry per repository the closure draws from, carrying the trust
# the resolve phase established for it. The build verifies each
# package's signature against these keys.
[[source]]
name             = "official"
signature_policy = "required"
trust_keys       = "[{\"fingerprint\":\"ef86…\",\"public_key\":\"…\",\"status\":\"active\"}]"
...
hash            = "sha256:1f3a…"
size_compressed = 4194304
size_installed  = 15728640

[[package]]
name            = "peiosutils"
version         = "0.9.1"
architecture    = "x86_64"
root            = "boot/initramfs"   # empty for the anchor
source          = "official"
url             = "https://pkgs.peios.org/pool/peiosutils-0.9.1.x86_64.peipkg"
hash            = "sha256:44ab…"
size_compressed = 2097152
size_installed  = 8388608
```

The two size fields are the §5.27 bounds, carried from the index entry
that selected the package: `size_compressed` bounds the download and
`size_installed` bounds decompression, and the archive's own manifest
must declare the same installed size. They are in the lock rather than
read from the archive because the archive's manifest lives *inside* the
stream being bounded — deriving a cap from it hands the cap to whoever
produced the bytes it defends against.
...
The *repository* trust chain — descriptor signature → index signature →
the name/version/hash mapping — is established once, at lock time, when
the closure is resolved. That much has always been the design. What the
lock now also carries is the **trust set itself**, so the build phase
can do the half that trust exists for: verify each package's inline
signature against the keys of its originating repository, under that
repository's signature policy, and refuse a key the descriptor marks
revoked or a transitioning key past its expiry.

Carrying only the hashes was not enough, and the gap was not obvious.
The hash proves the bytes are the ones the signed index listed — which
is exactly what a compromised signing key also achieves. So an operator
whose key was revoked in the descriptor was refused by `peipkg install`
and accepted by `peipkg-compose build`: the `revoked` status, the
format's in-band compromise response, was inert for every composed
image (PEI-373).

Key status is evaluated against the *build's* clock, not the lock's. A
build from an unchanged lock can therefore begin to fail after a key is
revoked, which is the intent rather than a wrinkle.

A `--locked` build still needs no descriptors and no network beyond
fetching the bytes — the keys travel in the lock — so air-gapped builds
are unaffected, and now verify signatures rather than inheriting
authenticity from a review of the lock.
...
The intended substitute was to record the resolver's
confirmation-required conditions as **annotations in the lock**, making
review of the lock the gate. That is **not implemented**: they are
printed to stderr and recorded nowhere, so a lock does not carry them
and reviewing it cannot surface them. The lock-schema field they need
is the deferred item at the end of this document.
...
- **`usr/share/licenses.json`** — the licence manifest, one entry per
  package in the closure.
- **Claim links** — the symlinks §5.23's claim declarations resolve to.
  compose materialises them at build time rather than leaving the first
  boot to do it, so a composed root is complete as it stands.
- **Every declared `[[root]]`**, including one no package was placed in.
  An empty declared root is a reasonable thing to want — somewhere to
  install into later — and registering a named root whose directory does
  not exist made `--root <name>` fail on the composed system when peipkg
  tried to open its database.
...
A package declaring `special_system_package` is still held to the
payload path rules unless the manifest also sets
`bypass_path_restrictions`. Two keys, two parties: the package says it
needs the waiver, the operator composing the image agrees to it. Neither
alone waives anything, and the check runs for **every** root rather than
only the anchor.
...
- **Extraction** writes payloads verbatim, with file metadata taken
  ...
  parallel.
- **The licence manifest** is generated from the closure, so it is
  determined by the lock like everything else.
...
1. ~~**Lock provenance.** Should the lock record which descriptor key
   fingerprints were trusted at lock time, for audit and review?~~
   **Answered — yes, and not optional.** The lock records the whole
   trust set, not just fingerprints, because the build phase needs the
   public keys to verify package signatures at all (PEI-373). Audit and
   review turned out to be the lesser reason.
...
- **Applying SD overrides** — compose writes plain files; SDs are
  established downstream. The consumer side now *validates* an override
  (that it names a real payload entry, that the entry is not a symlink,
  and that `sd` parses as a self-relative descriptor) but likewise does
  not apply one — see PEI-385.
